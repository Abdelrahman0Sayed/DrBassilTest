...
export default function EditArticle(article : getArticleResponse) {
  const [isDarkMode, setIsDarkMode] = useState(true);
  const [selectedImage, setSelectedImage] = useState(article.articleImg);
  const [title, setTitle] = useState(article.title);
  const [urlLink, setUrlLink] = useState(article.articleUrl);
  ...
  const handelEditArticle = async () => {
    if(title === '' || urlLink === '' || description === ''){
      alert('Please Fill All The Fields');
      return;
    }
    const body : EditArticleDto = {
      updatedArticleImg: selectedImage,
      updatedArticleUrl: urlLink,
      updatedArticleTitle: title,
      updatedDescription: description,
      currentArticleTitle: article.title
    }
    const adminService = new AdminService();
    const response = await adminService.editArticle(body);
    if(response?.ok){
    ...
    <div className="flex w-full justify-center">
      <div className="w-[90%] md:w-[80%] flex flex-col gap-10 text-[#03045e] dark:text-[#6EACDA] p-5">
        <div className="flex justify-center">
          <h1 className="text-3xl md:text-4xl text-center">Edit Your Article</h1>
        </div>
