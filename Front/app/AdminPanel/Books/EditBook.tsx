...
export default function EditBook(book : getBookResponse) {
  const [isDarkMode, setIsDarkMode] = useState(true);
  const [selectedImage, setSelectedImage] = useState(book.coverImage);
  const [title, setTitle] = useState(book.title);
  const [urlLink, setUrlLink] = useState(book.url);
  ...
  const handelEditBook = () => {
    const bookData: EditBookDto = {
      ...
      updatedBookTitle: title,
      updatedBookUrl: urlLink,
      updatedDescription: description,
      updatedChapters: chapters,
      updatedPages: pages,
      updatedBookImg: selectedImage,
    };
    adminService.editBook(bookData).then((res) => {
      if (res?.ok) {
    ...
    <div className="flex w-full justify-center">
      <div className="w-[90%] md:w-[80%] flex flex-col gap-10 text-[#03045e] dark:text-[#6EACDA] p-5">
        <div className="flex justify-center">
          <h1 className="text-3xl md:text-4xl text-center">Edit Your Book</h1>
        </div>
